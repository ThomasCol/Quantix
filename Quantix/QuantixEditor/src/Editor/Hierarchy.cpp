...
//}
//
//void Hierarchy::Update(std::string name, ImGuiWindowFlags flags, std::vector<Quantix::Physic::Transform3D*>& node)
//{
//	static int j = 0;
//	ImGui::Begin(name.c_str(), NULL, flags);
//	{
...
//void Hierarchy::DesactivatePrevInspector(std::vector<Node>& gameComponents, Node& gameComponent)
//{
...
void Hierarchy::Update(QXstring name, ImGuiWindowFlags flags, std::vector<Quantix::Physic::Transform3D*>& node)
{
	ImGui::Begin(name.c_str(), NULL, flags);
	{
		DrawObject(node, node);
