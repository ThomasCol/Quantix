...
#pragma region Functions
	...
	void Renderer::RenderColliders(std::vector<Components::ICollider*>& colliders)
	{
		...
		for (QXuint i = 0; i < colliders.size(); ++i)
		{
			...
			trs = Math::QXmat4::CreateTRSMatrix(obj->GetGlobalPosition() + colliders[i]->GetPosition(), obj->GetGlobalRotation().ConjugateQuaternion() * colliders[i]->GetRotation().ConjugateQuaternion(),
				colliders[i]->scale);
