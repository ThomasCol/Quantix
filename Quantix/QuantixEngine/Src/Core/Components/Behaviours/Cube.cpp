		...
		for (QXuint i = 0; i < overlapedObjects.size(); i++)
		{
			...
			if (cube)
			{
				if ((gameobject->GetGlobalPosition() - overlapedObjects[i]->GetLocalPosition()).Length() > 1.f)
				{
					...
					if (rigid)
						rigid->AddForce((gameobject->GetGlobalPosition() - overlapedObjects[i]->GetLocalPosition()) * (QXfloat)deltaTime * 1000.f);
				}
			}
		}
	}
	...
	void Cube::Reject(QXdouble deltaTime)
	{
		...
		for (QXuint i = 0; i < overlapedObjects.size(); i++)
		{
			...
			if (cube)
			{
				...
				if (rigid)
					rigid->AddForce( (overlapedObjects[i]->GetLocalPosition() - gameobject->GetGlobalPosition()) * (QXfloat)deltaTime * 1000.f);
			}
		}
	}
